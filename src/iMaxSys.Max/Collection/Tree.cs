...
public class Tree : TreeNode
{
    ...
    private static TreeNode SetNode(TreeStore store)
    {
        return new TreeNode(store.Id, store.Name, store.Action, store.Value, store.Description, store.Alias, store.Code, store.QuickCode, store.Data, store.Style, store.SelectedStyle, store.Icon, store.SelectedIcon, store.Ext, store.XppId, store.TenantId, store.Status);
    }
}
